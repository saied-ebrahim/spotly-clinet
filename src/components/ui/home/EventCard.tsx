      ...
      >
          ...
          <div className="absolute flex gap-1 bottom-3 left-3 flex-wrap">
            {event.category.length > 0 &&
              event.category.map((category) => (
                <span
                  key={category._id}
                  className={`text-[10px] uppercase font-semibold tracking-wider px-2 py-1 rounded-sm ${getCategoryColor(
                    category.name
        ...
        <div className="p-3 sm:p-4 relative flex gap-3 sm:gap-4 flex-1">
          ...
          <div className="flex flex-col gap-1 grow font-medium min-w-0 pr-3">
            ...
            <div
              className={`flex items-center justify-between mt-auto pt-3 border-t border-dashed border-gray-100 w-full `}
            >
              {/* Price Section */}
              <div
